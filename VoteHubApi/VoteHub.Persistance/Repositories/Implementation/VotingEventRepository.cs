...
{
    public class VotingEventRepository : IVotingEventRepository
    {
        private readonly VotingAppDbContext _context;

        public VotingEventRepository(VotingAppDbContext context)
        {
            _context = context;
        }
        ...
        public IQueryable<VotingEvent> Query()
        {
            return _context.Set<VotingEvent>().AsQueryable();
        }

        public async Task<IEnumerable<VotingEvent>> GetAllAsync()
        {
            return await _context.VotingEvents.ToListAsync();
        }
